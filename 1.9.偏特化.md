...
全特化版本的TypeChecker没有声明任何模板参数，而偏特化版本的TypeChecker声明了模板参数U。在全特化时，由于两个模板参数T、U的类型已经完全确定(本例中为int)，因此已经没有模板参数存在的必要，所以全特化时的模板参数声明是留空的；而在偏特化时，由于我们只给出了一部分类型或者特征，所以模板中还存在着一部分类型是不确定的，而这部分不确定的类型就需要依靠模板参数占位，这就是我们需要声明模板参数的原因。



## 偏特化参数与模板参数推断

在偏特化时，我们还需注意给定的偏特化参数能让编译器推断出所有的模板参数，例如：
...
对于偏特化参数`<int, U&>`，当我们以`TypeChecker<int, double&>`实例化模板时，编译器可以根据偏特化参数`U&`和具体类型`double&`推断出模板参数U为double。相反，对于下方的偏特化例子而言：
...
由于偏特化参数`<int, U&>`中没有涉及到任何有关模板参数T的信息，因此编译器无法推断模板参数T对应的类型。